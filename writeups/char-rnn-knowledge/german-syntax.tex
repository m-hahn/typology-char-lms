...
To build the test set, we select all nominative singular nouns from
the German UD treebank \textbf{(how many in total?)} (OOV noun ratios
...
\caption{Accuracy in the German syntax tasks, in function of number of intervening elements.}\label{fig:german-syntax}
\end{figure*}
...
the two cases (\textbf{how many in total?}).  We created four conditions, varying the amount of
...
Note also that we embed test phrases into full sentences. We
...
interest. \textbf{Add a full stimulus example (with the wrapping
...
the preposition are removed (thus, the example sentence above would
become: \textbf{add same example with prefix removed}).  If a model's
