...
We restricted to adjectives that occurred at least 100 times, and which did not end in -\emph{r}, as the latter ones often represent lemmatization problems.\footnote{TreeTagger occasionally did not remove the inflectional suffix -\emph{r} when lemmatizing adjectives.}
...
and is extremely frequent in German and in the Wikipedia corpus we are using.
...
prepositional phrase in the German Universal Dependencies treebank, subject to the
...
separated from the rest by the verb.} %\textbf{(please explain the latter more clearly)}.
