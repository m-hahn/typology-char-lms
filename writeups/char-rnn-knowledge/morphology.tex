...
tagged in the corpus by TreeTagger. Since in many cases verbs and nouns are cued by
...
they are virtually at chance level, similar to th autoencoder. %and \emph{worse} than the
%autoencoder.
...
We thus hypothesized that the CNLM-based classifier detects the presence of -\emph{e}- towards the end of the word, which would explain why it does well on -\emph{r} plurals and at chance of Umlaut plurals.

To test this hypothesis, we created a `classifier' that classsifies a word as plural if one of the last two characters is \emph{e}.
...
TODO confusion matrix is also closely matched?
...
For singulars, we balanced the incidence for each of the classes.
Resulting accuracies are shown in Table~\ref{tab:number-results-e}.

Performance of the diagnostic classifier for the LSTM CNLM is weaker than before for -r plurals, and stronger than before for Umlaut plurals.
...
The autoencoder performs essentially at chance on held-out classes.
This suggests that the CNLM encodes something more abstract than the autoencoder, but, overall, there is only modest evidence that the CNLM encodes any abstract notion of nominal plural in its hidden state.
