...
following standard practice in word segmentation, we trained the
classifier on a sequence of 1,000 characters and tested it on 1M
additional characters of running text, both taken from the left-out
Wikipedia test partitions. We report F1
performance on this task. We also considered a more cogent evaluation
regime, in which we split training and test data so that the number of
boundary and non-boundary conditions are balanced, and there is no
overlap between test and training words\ldots \textbf{TBC} To train
and test the classifier, we randomly selected positions from the test
partition of the Wikipedia corpus, such that half of these were the
last character of a token, and the other half were not.  We collected
1,000 such points as training data for the diagnostic classifiers, and
used 1M aditional datapoints for testing.  Both training and test sets
of the classifier were balanced between boundaries and word-internal
positions.  We sampled the test data points subject to the constraint
that the word (in the case of a boundary position) or word prefix (in
the case of a word-internal position) ending at the selected character
does not overlap with the training set. This ensures that a classifier
cannot succeed by looking for encodings reflecting specific words.
For each datapoint, we fed a substring of the 40 preceding characters
to the CNLM. %
...
For reference, we also train a diagnostic classifier on the
\emph{full} hidden layer of the LSTMs.

Results are shown in Table~\ref{tab:segmentation-results}. The
LSTM-based full-hidden-layer classifier has strong performance in
all 3 languages, confirming that the model encodes boundary information. Moreover, in all languages, a large proportion of this
performance is already accounted for by the single-parameter
classifier using boundary unit activations (the latter even
outperforms the full classifier in German). This confirms that
...
classifier in all languages. RNN performance, while above chance, is lagging behind. The weaker performance of RNNs
...
\paragraph{Error analysis} As a final way to characterize the function and behaviour of the
boundary units, we inspected the most frequent under- and
over-segmentation errors made by the classifier based on the single boundary units. We
...
on average shorter and morphology more limited). Considering first the 30 most common undersegmentations, the large
