...
information. We probe whether CNLMs are capable to track at least the
shallow form of semantics that is required in a fill-the-missing-word
test. In particular, we turn here to English, as for this language we
can use the Microsoft Research Sentence Completion task
\cite{Zweig:Burges:2011}. The challenge consists of sentences with a
gap, and a 5-word multiple choice to fill the gap. We chose it because
language models can be directly applied to it, by calculating the
likelihood of sentence variants with all possible completions, and
...
example, to make the right choice in \emph{``Was she his [
  \underline{client}|musings|discomfiture|choice|opportunity], his
  friend, or his mistress?}, one must realize that the missing word is
coordinated with \emph{friend} and \emph{mistress}, and that the
latter are animate entities.

Note that the task domain (Sherlock Holmes novels) is very different
from the Wikipedia data-set we originally trained our models on. For a
fairer comparison with previous work, we also re-train our models on
the training corpus provided with the benchmark, and consisting of
\textbf{how many?} words from 19th century English novels (we remove
...
confirm the importance of in-domain training, as all our models
trained on Wikipedia perform poorly (but still above chance level,
which is at 20\%).  When trained in domain, the LSTM CNLM outperforms
many earlier word-level neural models, and is only slightly below the
...
while still below LSTMs, is much stronger. Once more, this suggests
that word-level generalizations can be captured by a dictionary-less
character model, but the long-span processing abilities of LSTMs are
crucial for such models to succeed.
